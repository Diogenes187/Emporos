...
class CampaignReader:
    ...
    def campaign(self, public_id: str) -> dict[str, Any] | None:
        ...
        with self._connect() as connection:
            ...
            passenger_bookings=connection.execute("""SELECT journey.name AS journey_name,ship.name AS ship_name,receipt.passage_class,receipt.passenger_count,receipt.total_fare_minor FROM cmd_passenger_booking_receipt receipt JOIN journey_journey journey ON journey.journey_id=receipt.journey_id JOIN ship_ship ship ON ship.ship_id=receipt.ship_id WHERE receipt.campaign_id=%s ORDER BY receipt.command_id DESC""",(campaign_id,)).fetchall()
        return {
            **campaign,
            "actors": actors,
            "ships": ships,

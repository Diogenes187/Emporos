...
class CampaignReader:
    ...
    def campaign(self, public_id: str) -> dict[str, Any] | None:
        ...
        with self._connect() as connection:
            ...
            source_documents=connection.execute("SELECT document.public_id::text AS public_id,document.title,document.source_kind,document.original_filename,document.media_type,document.byte_count,document.page_count,document.ingestion_status,count(*) FILTER(WHERE page.review_status='verified') AS verified_pages,count(*) FILTER(WHERE page.visual_review_required) AS visual_review_pages FROM camp_source_document document JOIN camp_source_page page USING(source_document_id,campaign_id) WHERE document.campaign_id=%s GROUP BY document.source_document_id ORDER BY document.uploaded_at DESC",(campaign_id,)).fetchall()
        return {
            **campaign,
            "actors": actors,
            "ships": ships,

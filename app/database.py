    ...
    def campaign(self, public_id: str) -> dict[str, Any] | None:
        ...
        with self._connect() as connection:
            ...
            for actor in actors:
                ...
                term_step=connection.execute("""SELECT stint.career_stint_id,stint.terms_completed,career_rule.name AS career_name FROM actor_career_stint stint JOIN rule_rule career_rule ON career_rule.rule_id=stint.career_rule_id WHERE stint.actor_id=%s AND stint.stint_status='active' AND stint.rank_zero_award_completed ORDER BY stint.stint_order DESC LIMIT 1""",(actor_id,)).fetchone()
                if term_step:

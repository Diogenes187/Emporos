    ...
    def campaign(self, public_id: str) -> dict[str, Any] | None:
        ...
        with self._connect() as connection:
            ...
            for encounter in encounters:
                encounter["social_participants"]=connection.execute("""SELECT actor.public_id::text AS actor_public_id,actor.name,participant.participant_role,actor.controller_reference=campaign.owner_reference AS player_controlled,attitude.attitude_code FROM enc_participant participant JOIN enc_encounter encounter USING(encounter_id) JOIN camp_campaign campaign USING(campaign_id) JOIN actor_actor actor USING(actor_id) LEFT JOIN enc_attitude_state state USING(encounter_id,actor_id) LEFT JOIN rule_attitude attitude ON attitude.rule_id=state.attitude_rule_id WHERE encounter.public_id=%s ORDER BY actor.name""",(encounter["public_id"],)).fetchall()

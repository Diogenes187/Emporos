"""Import paired-source Medical Treatment and CE-COMBAT-014."""
import argparse
import os
...
def main() -> int:
    ...
    session.headers["User-Agent"]="BaseCepheus medical importer/1.0"
    ...
    for phrase in (
        ...
        "if the check is failed the patient loses",
        "2 + the character's endurance dm",
        "divided evenly among all damaged characteristics",
    ):
        if any(normalize(phrase) not in text for text in paired):
            raise ValueError(f"Paired Medical Treatment sources omit: {phrase}")
    with psycopg.connect(dsn) as connection:
        ...
        rule_id=publish_rule(
            connection,package,"combat.medical-treatment",
            "Medical Treatment","combat",
            "First Aid, Surgery, and daily Medical Care.")
        payload={
            "first_aid":{"full_minutes":5,"late_minutes":60,
                         "multipliers":[2,1],"self_dm":-2},
            "surgery":{"success_multiplier":2,"self_dm":-4,
                       "facility_required":True},
            "medical_care":{"formula":"max(0,2+Endurance DM+Medicine)",
                            "even_allocation":True},
            "cross_species_dm":-2,
        }
        for side in ("github","ogn"):
            ...
            locator=upsert_locator(
                ...
                "Personal Combat > Injury and Recovery > Medical Treatment",
                "personal-medical-treatment","Medical Treatment",0)
            candidate,review=stage_candidate(
                connection,batch,artifact,locator,"combat",
                "combat.medical-treatment",payload)
            add_provenance(
                connection,rule_id,package,locator,candidate,review,
                "direct" if side=="github" else "corroborating",
                side=="github")
        connection.execute(
            """INSERT INTO rule_personal_medical_treatment
               VALUES (%s,8,5,60,2,1,-2,2,-4,-2,2)
        ...
        connection.execute(
            """INSERT INTO rule_interpretation
               ...
               VALUES (%s,'agreed_interpretation',%s,'CE-COMBAT-014')
               ON CONFLICT DO NOTHING""",
            (rule_id,"Raymond approved Average checks, absolute failed-surgery "
             "loss, injury-linked one-time First Aid, relational facilities, "
             "and controller-assigned even-allocation remainders."))
        connection.execute(
    ...
    print("published Medical Treatment and CE-COMBAT-014")
    return 0

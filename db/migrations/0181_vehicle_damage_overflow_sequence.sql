            ...
            IF NOT FOUND
               OR rolled_system.location_code<>
                ...
                RAISE EXCEPTION
                    ...
                    USING ERRCODE='23514';
            END IF;
            IF damage_hit.resolution_kind='overflow' THEN
                SELECT count(*) INTO prior_direct_hits
                FROM venc_damage_location_hit prior_hit
                WHERE prior_hit.vehicle_damage_application_id=
                      OLD.vehicle_damage_application_id
                  AND prior_hit.hit_order<damage_hit.hit_order
                  AND prior_hit.resolution_kind='direct'
                  AND prior_hit.vehicle_system_state_id=
                      damage_hit.rolled_vehicle_system_state_id;
                IF rolled_system.current_hits+
                   prior_direct_hits<maximum_hits THEN
                    RAISE EXCEPTION

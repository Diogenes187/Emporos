CREATE TABLE actor_character_profile_revision (
    character_profile_revision_id bigint GENERATED ALWAYS AS IDENTITY
        PRIMARY KEY,
    ...
    gender_identity text CHECK (
        gender_identity IS NULL OR btrim(gender_identity) <> ''
    ),
    appearance text CHECK (appearance IS NULL OR btrim(appearance) <> ''),
    created_at timestamptz NOT NULL DEFAULT clock_timestamp(),
    UNIQUE (actor_id,revision_number)
);

CREATE TABLE actor_character_personal_goal (
    character_profile_revision_id bigint NOT NULL REFERENCES
        actor_character_profile_revision(character_profile_revision_id),
    goal_order smallint NOT NULL CHECK (goal_order > 0),
    goal_text text NOT NULL CHECK (btrim(goal_text) <> ''),
    PRIMARY KEY (character_profile_revision_id,goal_order)
);

CREATE VIEW actor_current_character_profile AS
SELECT DISTINCT ON (profile.actor_id)
       profile.character_profile_revision_id,profile.actor_id,
       profile.revision_number,profile.character_name,
       profile.gender_identity,profile.appearance,profile.created_at
FROM actor_character_profile_revision profile
ORDER BY profile.actor_id,profile.revision_number DESC;

CREATE TABLE cmd_character_final_details_receipt (
    command_id bigint PRIMARY KEY REFERENCES cmd_command(command_id),
    character_profile_revision_id bigint NOT NULL UNIQUE REFERENCES
        actor_character_profile_revision(character_profile_revision_id),

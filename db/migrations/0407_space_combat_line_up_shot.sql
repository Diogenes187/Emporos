...
SELECT artifact.source_work_id,artifact.source_artifact_id,'heading',
 'Space Combat > Significant Actions > Line Up the Shot',
 CASE work.work_code WHEN 'cepheus-engine.ogn' THEN 'Cepheus Engine SRD, Space Combat: Line Up the Shot'
 ELSE 'Cepheus Engine v9.1, Space Combat: Line Up the Shot' END
...
SELECT content_package_id,'combat.space.line-up-shot','Line Up the Shot','combat','approved',
 'Pilot action providing a stable firing platform and current-round attack bonus.' FROM package;

CREATE TABLE rule_space_combat_line_up_shot(
 ...
 success_attack_bonus smallint NOT NULL CHECK(success_attack_bonus=1),
 exceptional_effect_threshold smallint NOT NULL CHECK(exceptional_effect_threshold=6),
 exceptional_attack_bonus smallint NOT NULL CHECK(exceptional_attack_bonus=2),
 applies_to_all_vessel_attacks boolean NOT NULL,
 applies_current_round_only boolean NOT NULL,
 failure_consumes_action boolean NOT NULL
);
INSERT INTO rule_space_combat_line_up_shot
...
WHERE action.rule_code='combat.space.line-up-shot' AND skill.rule_code='skill.piloting'
...
WHERE rule.rule_code='combat.space.line-up-shot'
 AND locator.heading_path='Space Combat > Significant Actions > Line Up the Shot'
 AND work.work_code IN('cepheus-engine.ogn','cepheus-engine.github-v9.1');

CREATE TABLE senc_line_up_shot_receipt(
 line_up_shot_receipt_id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
 ...
 senc_vessel_id bigint NOT NULL,action_id bigint NOT NULL UNIQUE,
 pilot_assignment_id bigint NOT NULL,pilot_ship_id bigint NOT NULL,
 task_command_id bigint NOT NULL UNIQUE REFERENCES cmd_actor_task_receipt(command_id),
 task_effect smallint NOT NULL,task_succeeded boolean NOT NULL,
 attack_bonus smallint NOT NULL CHECK(attack_bonus IN(0,1,2)),
...
CREATE FUNCTION senc_validate_line_up_shot_receipt() RETURNS trigger LANGUAGE plpgsql AS $$
...
BEGIN
 SELECT action.action_code,action.space_combat_round_id,turn.senc_vessel_id,turn.crew_assignment_id,
  assignment.ship_id,assignment.actor_id,assignment.duty_status,definition.position_code INTO action_row
 ...
 IF action_row.action_code<>'line-up-shot' OR action_row.space_combat_round_id<>NEW.space_combat_round_id
  ...
  OR task_row.actor_id<>action_row.actor_id OR task_row.skill_rule_id<>piloting
  OR task_row.difficulty_rule_id<>average OR task_row.effect<>NEW.task_effect
  OR task_row.succeeded<>NEW.task_succeeded OR actual_round<>NEW.round_number THEN
  RAISE EXCEPTION 'Line Up the Shot receipt does not match its active Pilot action and Piloting check' USING ERRCODE='23514';
 END IF;
 RETURN NEW;
END $$;
CREATE TRIGGER senc_line_up_shot_valid BEFORE INSERT ON senc_line_up_shot_receipt
FOR EACH ROW EXECUTE FUNCTION senc_validate_line_up_shot_receipt();
CREATE FUNCTION senc_reject_line_up_shot_mutation() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN RAISE EXCEPTION 'Line Up the Shot receipts are immutable'; END $$;
CREATE TRIGGER senc_line_up_shot_immutable BEFORE UPDATE OR DELETE ON senc_line_up_shot_receipt
FOR EACH ROW EXECUTE FUNCTION senc_reject_line_up_shot_mutation();

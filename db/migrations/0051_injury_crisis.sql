CREATE TABLE actor_career_injury_crisis (
    injury_crisis_id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    ...
    medical_cost_die smallint NOT NULL CHECK (medical_cost_die BETWEEN 1 AND 6),
...
CREATE TABLE cmd_injury_crisis_cost_receipt (
    command_id bigint PRIMARY KEY REFERENCES cmd_command(command_id),
    injury_crisis_id bigint NOT NULL UNIQUE REFERENCES actor_career_injury_crisis(
        injury_crisis_id
    )
);

CREATE TABLE cmd_injury_crisis_resolution_receipt (
    command_id bigint PRIMARY KEY REFERENCES cmd_command(command_id),
    injury_crisis_id bigint NOT NULL UNIQUE REFERENCES actor_career_injury_crisis(
        injury_crisis_id
    ),
    ...
    CHECK (
        ...
        OR
        (resolution_kind='accept_death' AND cash_after=cash_before)
    )
);

CREATE TABLE cmd_injury_crisis_restoration (
    command_id bigint NOT NULL REFERENCES cmd_injury_crisis_resolution_receipt(
        command_id
    ),
...
ALTER TABLE actor_lifepath_state
    DROP CONSTRAINT actor_lifepath_state_lifepath_status_check;
ALTER TABLE actor_lifepath_state
    ADD CONSTRAINT actor_lifepath_state_lifepath_status_check CHECK (
        lifepath_status IN ('active','completed','deceased')
    );

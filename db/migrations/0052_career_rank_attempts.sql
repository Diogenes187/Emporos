ALTER TABLE actor_career_term
    ADD COLUMN bonus_training_rolls smallint NOT NULL DEFAULT 0
        CHECK (bonus_training_rolls >= 0);

CREATE TABLE cmd_career_rank_attempt_receipt (
    command_id bigint PRIMARY KEY REFERENCES cmd_command(command_id),
    career_term_id bigint NOT NULL REFERENCES actor_career_term(career_term_id),
    attempt_kind text NOT NULL CHECK (
        attempt_kind IN ('commission','advancement')
    ),
    decision text NOT NULL CHECK (decision IN ('attempt','decline')),
    characteristic_rule_id bigint REFERENCES rule_characteristic(rule_id),
    characteristic_value smallint,
    characteristic_modifier smallint,
    target_number smallint,
    check_total smallint,
    succeeded boolean,
    prior_rank smallint NOT NULL CHECK (prior_rank BETWEEN 0 AND 6),
    resulting_rank smallint NOT NULL CHECK (resulting_rank BETWEEN 0 AND 6),
    awarded_career_rank_id bigint REFERENCES rule_career_rank(career_rank_id),
    source_skill_rule_id bigint REFERENCES rule_skill(rule_id),
    granted_skill_rule_id bigint REFERENCES rule_skill(rule_id),
    prior_skill_level smallint,
    resulting_skill_level smallint,
    UNIQUE (career_term_id,attempt_kind),
    CHECK (
        (decision='decline' AND characteristic_rule_id IS NULL
         AND characteristic_value IS NULL
         AND characteristic_modifier IS NULL AND target_number IS NULL
         AND check_total IS NULL AND succeeded IS NULL
         AND prior_rank=resulting_rank)
        OR
        (decision='attempt' AND characteristic_rule_id IS NOT NULL
         AND characteristic_value IS NOT NULL
         AND characteristic_modifier IS NOT NULL AND target_number IS NOT NULL
         AND check_total IS NOT NULL AND succeeded IS NOT NULL)
    )
);

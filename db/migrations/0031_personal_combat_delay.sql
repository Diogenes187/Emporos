CREATE TABLE rule_personal_delay (
    rule_id             bigint PRIMARY KEY REFERENCES rule_rule(rule_id),
    may_act_later_in_round boolean NOT NULL,
    may_interrupt_action boolean NOT NULL,
    initiative_becomes_action_count boolean NOT NULL,
    may_forfeit_for_first_next_round boolean NOT NULL,
    next_round_initiative_above_current_first smallint NOT NULL CHECK (
                            next_round_initiative_above_current_first > 0
                        )
);

ALTER TABLE enc_personal_combatant
    ADD COLUMN delayed_this_round boolean NOT NULL DEFAULT false;
...
CREATE TABLE cmd_personal_delay_receipt (
    ...
    initiative_when_delayed integer NOT NULL,
...
CREATE TABLE cmd_personal_delay_resume_receipt (
    command_id          bigint PRIMARY KEY REFERENCES cmd_command(command_id),
    encounter_id        bigint NOT NULL REFERENCES enc_personal_combat(encounter_id),
    actor_id            bigint NOT NULL REFERENCES actor_actor(actor_id),
    round_number        integer NOT NULL CHECK (round_number > 0),
    ...
    interrupted_actor_id bigint REFERENCES actor_actor(actor_id),
    FOREIGN KEY (encounter_id, actor_id)
        REFERENCES enc_personal_combatant(encounter_id, actor_id),
    CHECK (interrupted_actor_id IS NULL OR interrupted_actor_id <> actor_id),
    UNIQUE (encounter_id, actor_id, round_number)
);

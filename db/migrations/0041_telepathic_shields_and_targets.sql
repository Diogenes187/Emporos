CREATE TABLE psi_power_targeting (
    power_rule_id bigint PRIMARY KEY REFERENCES psi_power(power_rule_id),
    target_kind text NOT NULL CHECK (
        target_kind IN (
            'self', 'actor', 'area', 'location',
            'object_or_creature', 'passive'
        )
    )
);

CREATE TABLE actor_telepathic_shield_state (
    actor_id bigint PRIMARY KEY REFERENCES actor_actor(actor_id),
    shield_raised boolean NOT NULL DEFAULT true,
...
CREATE TABLE cmd_telepathic_shield_receipt (
    command_id bigint PRIMARY KEY REFERENCES cmd_command(command_id),
    actor_id bigint NOT NULL REFERENCES actor_actor(actor_id),
    shield_before boolean NOT NULL,
    shield_after boolean NOT NULL
);

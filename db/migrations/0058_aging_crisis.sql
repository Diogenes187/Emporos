ALTER TABLE actor_lifepath_state
    ADD COLUMN qualification_barred boolean NOT NULL DEFAULT false;

UPDATE actor_lifepath_state lifepath
SET qualification_barred=true
WHERE EXISTS (
    SELECT 1 FROM actor_career_injury_crisis crisis
    WHERE crisis.actor_id=lifepath.actor_id
      AND crisis.crisis_status='paid'
);

CREATE TABLE actor_career_aging_crisis (
    aging_crisis_id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    ...
    medical_cost_die smallint NOT NULL CHECK (
        medical_cost_die BETWEEN 1 AND 6
    ),
    medical_cost_credits integer NOT NULL CHECK (
        medical_cost_credits BETWEEN 10000 AND 60000
        AND medical_cost_credits % 10000 = 0
    ),
    crisis_status text NOT NULL DEFAULT 'awaiting_decision' CHECK (
        crisis_status IN ('awaiting_decision','paid','death')
    )
);

CREATE TABLE cmd_aging_crisis_cost_receipt (
    command_id bigint PRIMARY KEY REFERENCES cmd_command(command_id),
    aging_crisis_id bigint NOT NULL UNIQUE REFERENCES actor_career_aging_crisis(
        aging_crisis_id
    )
);

CREATE TABLE cmd_aging_crisis_resolution_receipt (
    command_id bigint PRIMARY KEY REFERENCES cmd_command(command_id),
    aging_crisis_id bigint NOT NULL UNIQUE REFERENCES actor_career_aging_crisis(
        aging_crisis_id
    ),
    resolution_kind text NOT NULL CHECK (
        resolution_kind IN ('pay','accept_death')
    ),
    cash_before bigint NOT NULL,
    cash_after bigint NOT NULL,
    CHECK (
        (resolution_kind='pay' AND cash_after <= cash_before)
        OR (resolution_kind='accept_death' AND cash_after=cash_before)
    )
);

CREATE TABLE cmd_aging_crisis_restoration (
    command_id bigint NOT NULL REFERENCES cmd_aging_crisis_resolution_receipt(
        command_id
    ),

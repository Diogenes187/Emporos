ALTER TABLE actor_career_stint
    ADD COLUMN rank_zero_award_completed boolean NOT NULL DEFAULT false;
...
ALTER TABLE cmd_domain_event ADD CONSTRAINT cmd_domain_event_event_type_check CHECK (
    event_type IN (
        ...
        'career_rank_zero_award_applied'
    )
);

CREATE TABLE cmd_career_rank_zero_award_receipt (
    ...
    CHECK (
        (source_skill_rule_id IS NULL AND granted_skill_rule_id IS NULL
         AND prior_skill_level IS NULL AND resulting_skill_level IS NULL)
        OR
        (source_skill_rule_id IS NOT NULL AND granted_skill_rule_id IS NOT NULL
         AND resulting_skill_level IS NOT NULL
         AND resulting_skill_level >= 0)
    )
);

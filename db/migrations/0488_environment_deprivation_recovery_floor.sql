...
DECLARE actor bigint;maximum integer;locked integer;
BEGIN IF NEW.point_change<0 THEN RETURN NEW;END IF;
 ...
 SELECT maximum_value INTO STRICT maximum FROM actor_characteristic WHERE actor_id=actor AND characteristic_rule_id=NEW.characteristic_rule_id;
 ...
 IF NEW.value_after>greatest(0,maximum-locked) THEN RAISE EXCEPTION 'Healing cannot recover deprivation damage until the actor receives the required food or water' USING ERRCODE='23514';END IF;RETURN NEW;END $$;

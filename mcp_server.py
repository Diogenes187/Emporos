...
def _resume(args):
 ...
 with _connect() as c:
  ...
  clock=c.execute('SELECT day_number,second_of_day FROM camp_clock WHERE campaign_id=%s',(cid,)).fetchone()
  characters=[]
  for actor in c.execute("SELECT actor.actor_id,actor.public_id::text,actor.name,homeworld.name FROM actor_actor actor LEFT JOIN loc_location homeworld ON homeworld.location_id=actor.homeworld_location_id WHERE actor.campaign_id=%s AND actor.lifecycle_status='active' ORDER BY actor.name",(cid,)).fetchall():
   ...
   characters.append({'public_id':actor[1],'name':actor[2],'homeworld':actor[3],'characteristics':[{'name':r[0],'current':r[1],'maximum':r[2]} for r in characteristics],'skills':[{'name':r[0],'level':r[1]} for r in skills]})
  world=c.execute("""SELECT starting.public_id::text,starting.name,current.public_id::text,current.name
    FROM camp_campaign_setting setting
    LEFT JOIN loc_location starting ON starting.location_id=setting.starting_world_location_id
    LEFT JOIN loc_location current ON current.location_id=setting.current_world_location_id
    WHERE setting.campaign_id=%s""",(cid,)).fetchone()
  ...
  return {
   ...
   'clock':{'day_number':clock[0],'second_of_day':clock[1]},
   'world':{'starting':{'public_id':world[0],'name':world[1]} if world and world[0] else None,'current':{'public_id':world[2],'name':world[3]} if world and world[2] else None},
   'characters':characters,'ships':ships,
   ...
   'resume_instruction':(
    ...
    'The world.current field is the party location. A character homeworld is background only. Keyed adventure locations are a separate scale: never call enter_adventure_location unless active_adventure is non-null and the requested public ID appears in its locations. '
...
def _resume_latest(args=None):
 # Deliberately ignore all supplied arguments. Older MCP clients may retain a
 # stale optional-parameter schema, and small local models may emit its example
 # placeholders literally. The default resume operation must remain safe.
 return _resume({})
...
TOOLS={
 ...
 'campaign_resume':('THE parameterless startup/resume call for playing Emporos through an AI client. Call with an empty object. It ignores any accidentally supplied arguments, resumes the most recently active campaign, and remains its referee until the user changes campaigns or asks to stop.',{},(),_resume_latest),
 ...
 'enter_adventure_location':('Enter a location within the active keyed adventure only. Do not use this for a planet, star system, campaign start, or ordinary scene.',{'location_public_id':{'type':'string'},'idempotency_key':{'type':'string'}},('location_public_id',),_enter_location),

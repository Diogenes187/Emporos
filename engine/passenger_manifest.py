...
def _load(c,cid,pub,replayed):
 r=c.execute("SELECT journey.public_id,ship.public_id,receipt.passenger_count,receipt.stateroom_units_used,receipt.low_berths_used,receipt.steward_level_quanta,receipt.steward_quanta_required FROM cmd_passenger_manifest_receipt receipt JOIN journey_journey journey ON journey.journey_id=receipt.journey_id JOIN ship_ship ship ON ship.ship_id=receipt.ship_id WHERE receipt.command_id=%s",(cid,)).fetchone();return PassengerManifestResult(str(pub),str(r[0]),str(r[1]),r[2],r[3],r[4],r[5],r[6],replayed)
def finalize_passenger_manifest_command(c:psycopg.Connection,*,initiator_reference:str,idempotency_key:str,journey_public_id:str)->PassengerManifestResult:
 with c.transaction():
  ...
  locked=c.execute("SELECT journey.journey_id FROM journey_journey journey JOIN camp_campaign campaign ON campaign.campaign_id=journey.campaign_id WHERE journey.public_id=%s AND campaign.owner_reference=%s AND journey.journey_status='planning' FOR UPDATE OF journey",(journey_public_id,initiator_reference)).fetchone()
  if not locked:raise ValueError('A planning journey with assigned, booked passengers is required')
 ...
 FROM journey_journey journey
 JOIN camp_campaign campaign ON campaign.campaign_id=journey.campaign_id
 JOIN ship_ship ship ON ship.ship_id=journey.ship_id AND ship.campaign_id=journey.campaign_id
 JOIN journey_passage passage ON passage.journey_id=journey.journey_id AND passage.campaign_id=journey.campaign_id
 JOIN journey_active_passage_accommodation accommodation ON accommodation.journey_passage_id=passage.journey_passage_id AND accommodation.campaign_id=journey.campaign_id AND accommodation.journey_id=journey.journey_id
 ...
 GROUP BY journey.campaign_id,journey.journey_id,journey.ship_id,ship.public_id""",(journey_public_id,initiator_reference)).fetchone()

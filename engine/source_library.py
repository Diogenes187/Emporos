...
def ingest_campaign_source_command(c:psycopg.Connection,*,initiator_reference:str,idempotency_key:str,campaign_public_id:str,title:str,source_kind:str,original_filename:str,media_type:str,content:bytes,storage_root:Path)->SourceIngestionResult:
 ...
 with c.transaction():
  ...
  c.execute("INSERT INTO cmd_campaign_source_ingestion_receipt(command_id,campaign_id,source_document_id,page_count,extracted_page_count,review_page_count,ingestion_status) VALUES(%s,%s,%s,%s,%s,%s,%s)",(cid,campaign[0],document,len(pages),sum(1 for _,state,_ in pages if state=='extracted'),review,status));c.execute("INSERT INTO cmd_domain_event(command_id,event_order,event_type) VALUES(%s,1,'campaign_source_ingested')",(cid,));c.execute("UPDATE cmd_command SET command_status='completed',completed_at=clock_timestamp() WHERE command_id=%s",(cid,))

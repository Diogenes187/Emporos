...
def complete_spacecraft_journey_leg_command(c:psycopg.Connection,*,referee_reference:str,idempotency_key:str,journey_public_id:str,leg_order:int)->SpacecraftLegExecutionResult:
 with c.transaction():
  ...
  elif final:c.execute("UPDATE journey_journey SET journey_status='completed',ended_at=clock_timestamp() WHERE journey_id=%s",(s[0],));c.execute("UPDATE journey_participant SET commitment_status='completed',released_at=clock_timestamp() WHERE journey_id=%s AND commitment_status IN('planned','committed') AND actor_id NOT IN(SELECT actor_id FROM journey_passage WHERE journey_id=%s AND passage_class='low' AND passage_status='boarded')",(s[0],s[0]));c.execute("UPDATE journey_passage SET passage_status='completed',concurrency_version=concurrency_version+1 WHERE journey_id=%s AND passage_class<>'low' AND passage_status IN('booked','boarded')",(s[0],))
